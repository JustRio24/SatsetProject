...
<body class="bg-[#f8fafc] text-slate-900">
    <!-- Navigation -->
    <nav class="fixed top-0 w-full z-50 px-3 sm:px-6 py-4">
        <div class="max-w-7xl mx-auto glass rounded-[2rem] px-4 sm:px-8 py-4 flex justify-between items-center shadow-xl shadow-slate-200/50">
            ...
            <div class="flex items-center gap-4">
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/dashboard') }}" class="px-4 sm:px-6 py-2.5 sm:py-3 bg-slate-900 text-white rounded-xl sm:rounded-2xl font-bold text-[10px] sm:text-xs uppercase tracking-widest hover:bg-black transition-all shadow-xl">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="px-5 sm:px-8 py-2.5 sm:py-3 bg-red-600 text-white rounded-xl sm:rounded-2xl font-bold text-[10px] sm:text-xs uppercase tracking-widest hover:bg-red-700 transition-all shadow-xl shadow-red-500/20">Login</a>
                    @endauth
                @endif
            </div>
        </div>
    ...
    <header class="pt-40 pb-20 px-6 hero-bg overflow-hidden relative">
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div class="space-y-8 relative z-10 animate-fade-in-up">
                ...
                <h1 class="text-6xl md:text-7xl font-black text-slate-900 tracking-tighter leading-none">
                    Membangun Masa Depan Dengan <span class="gradient-text">SPECTRA.</span>
                </h1>
                <p class="text-lg text-slate-500 font-medium leading-relaxed max-w-xl">
                    SPECTRA <span class="text-red-600 font-black text-xs uppercase tracking-widest">(System for Project, Earnings, & Comprehensive Task Reporting Analysis)</span> adalah platform inti PT. SatSet MerahPutih Indonesia untuk efisiensi tenaga kerja dan operasional terpadu.
                </p>
    ...
    <section id="about" class="py-32 px-6 bg-white">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                <!-- Construction -->
                <div class="p-10 rounded-[3rem] bg-[#f8fafc] border border-slate-50 hover:bg-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group animate-fade-in-up">
                ...
                <!-- Workshop -->
                <div class="p-10 rounded-[3rem] bg-[#f8fafc] border border-slate-50 hover:bg-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group animate-fade-in-up delay-100">
                ...
                <!-- Entertainment -->
                <div class="p-10 rounded-[3rem] bg-[#f8fafc] border border-slate-50 hover:bg-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group animate-fade-in-up delay-200">
    ...
    <section id="contact" class="py-20 px-6">
        <div class="max-w-5xl mx-auto bg-slate-900 rounded-[3.5rem] p-12 md:p-20 text-center relative overflow-hidden shadow-2xl">
            <div class="relative z-10 space-y-8">
                ...
                <div class="pt-4">
                    <a href="{{ route('login') }}" class="px-1 py-2 bg-white text-slate-900 rounded-3xl font-black text-sm uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-xl shadow-white/5">Akses Dashboard SPECTRA</a>
                </div>
            </div>
